const chartData = {
  userToday: 1211019,
  userLastDay: 1000000,
  userLastMonth: 800000,
  userGrowthLastDay: "123.58",
  userGrowthLastMonth: "179.47",
  age: [
    {
      key: "0-20",
      value: 429149,
    },
    {
      key: "20-30",
      value: 629219,
    },
    {
      key: "30-50",
      value: 849026,
    },
    {
      key: ">50",
      value: 328755,
    },
  ],
  averageAge: "30.33",
  totalDevices: 1624523,
  devices: [
    {
      key: "Android",
      value: 607569,
    },
    {
      key: "iOS",
      value: 558434,
    },
    {
      key: "PC",
      value: 458520,
    },
  ],
  gender: [
    {
      key: "male",
      value: 1117318,
    },
    {
      key: "female",
      value: 1118831,
    },
  ],
  rider: {
    axisX: ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"],
    orderData: {
      legend1: "去年骑手月人均接单",
      legend2: "今年骑手月人均接单",
      data1: ["330", "420", "560", "450", "610", "890", "720", "610", "580", "750", "770", "600"],
      data2: ["430", "510", "660", "550", "710", "990", "620", "550", "760", "810", "930", "720"],
    },
    rateData: {
      legend1: "去年月新增骑手",
      legend2: "今年月新增骑手",
      data1: ["129", "223", "202", "197", "300", "112", "333", "249", "178", "322", "401", "167"],
      data2: ["179", "263", "282", "297", "330", "344", "222", "299", "190", "455", "566", "233"],
    },
  },
  category: {
    data1: {
      axisX: ["粉面粥店", "简餐便当", "汉堡披萨", "香锅冒菜", "小吃炸串", "地方菜系", "轻食简餐"],
      data1: [76, 19, 47, 62, 42, 14, 53],
      data2: [24, 81, 53, 38, 58, 86, 47],
    },
    data2: {
      axisX: ["草莓", "甘蔗", "榴莲", "菠萝", "香蕉", "梨", "苹果"],
      data1: [84, 27, 63, 60, 16, 52, 81],
      data2: [16, 73, 37, 40, 84, 48, 19],
    },
  },
  orderGrowthLastDay: "122.86",
  orderGrowthLastMonth: "178.58",
  orderLastMonth: 1600000,
  orderLastDay: 2000000,
  orderToday: 4457205,
  orderTrend: [620, 432, 220, 534, 790, 430, 220, 320, 532, 320, 834, 690, 530, 220, 620],
  orderUserTrend: [410, 82, 200, 334, 390, 330, 220, 150, 82, 200, 134, 290, 330, 150],
  orderUserTrendAxis: [
    "00:00",
    "01:00",
    "02:00",
    "03:00",
    "04:00",
    "05:00",
    "06:00",
    "07:00",
    "08:00",
    "09:00",
    "10:00",
    "11:00",
    "12:00",
    "13:00",
  ],
  orderFullYearAxis: ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"],
  orderFullYear: [410, 82, 200, 334, 390, 330, 220, 150, 82, 200, 134, 290],
  userFullYearAxis: ["1月", "2月", "3月", "4月", "5月", "6月", "7月", "8月", "9月", "10月", "11月", "12月"],
  userFullYear: [110, 120, 90, 220, 175, 212, 192, 95, 88, 120, 250, 310],
  orderRank: [
    {
      no: 1,
      name: "肯德基",
      money: "323,234",
    },
    {
      no: 2,
      name: "麦当劳",
      money: "299,132",
    },
    {
      no: 3,
      name: "肯德基",
      money: "283,998",
    },
    {
      no: 4,
      name: "海底捞",
      money: "266,223",
    },
    {
      no: 5,
      name: "西贝筱面村",
      money: "223,445",
    },
    {
      no: 6,
      name: "汉堡王",
      money: "219,663",
    },
    {
      no: 7,
      name: "真功夫",
      money: "200,997",
    },
  ],
  userRank: [
    {
      no: 1,
      name: "麦当劳",
      money: "211,335",
    },
    {
      no: 2,
      name: "肯德基",
      money: "210,597",
    },
    {
      no: 3,
      name: "必胜客",
      money: "200,998",
    },
    {
      no: 4,
      name: "海底捞",
      money: "199,220",
    },
    {
      no: 5,
      name: "西贝筱面村",
      money: "195,444",
    },
    {
      no: 6,
      name: "汉堡王",
      money: "180,161",
    },
    {
      no: 7,
      name: "真功夫",
      money: "172,995",
    },
  ],
  salesGrowthLastDay: "147.90",
  salesGrowthLastMonth: "220.56",
  salesLastMonth: 23200000,
  salesLastDay: 30000000,
  salesToday: 74369603,
  orderPrice: 20,
  orderUser: 2236149,
  covertRate: "14.86",
  returnRate: "5.40",
  areaSales: [
    {
      order: "北京 -6%",
      shop: "北京 -11%",
      rider: "北京 -14%",
      newShop: "北京 +15%",
      avgOrder: "北京 +12%",
    },
    {
      order: "上海 -17%",
      shop: "上海 -11%",
      rider: "上海 -20%",
      newShop: "上海 +9%",
      avgOrder: "上海 +14%",
    },
    {
      order: "广州 -5%",
      shop: "广州 -12%",
      rider: "广州 -21%",
      newShop: "广州 -15%",
      avgOrder: "广州 +16%",
    },
    {
      order: "深圳 +11%",
      shop: "深圳 +17%",
      rider: "深圳 +9%",
      newShop: "深圳 +21%",
      avgOrder: "深圳 -19%",
    },
    {
      order: "南京 -8%",
      shop: "南京 -13%",
      rider: "南京 -18%",
      newShop: "南京 -17%",
      avgOrder: "南京 +3%",
    },
    {
      order: "杭州 +14%",
      shop: "杭州 -16%",
      rider: "杭州 -3%",
      newShop: "杭州 -13%",
      avgOrder: "杭州 -19%",
    },
    {
      order: "合肥 +13%",
      shop: "合肥 -7%",
      rider: "合肥 +7%",
      newShop: "合肥 +20%",
      avgOrder: "合肥 +15%",
    },
    {
      order: "济南 +8%",
      shop: "济南 +18%",
      rider: "济南 +15%",
      newShop: "济南 -11%",
      avgOrder: "济南 +7%",
    },
    {
      order: "太原 +7%",
      shop: "太原 +5%",
      rider: "太原 -10%",
      newShop: "太原 +6%",
      avgOrder: "太原 -15%",
    },
    {
      order: "成都 -22%",
      shop: "成都 +8%",
      rider: "成都 -5%",
      newShop: "成都 -14%",
      avgOrder: "成都 -10%",
    },
    {
      order: "重庆 -21%",
      shop: "重庆 -16%",
      rider: "重庆 -5%",
      newShop: "重庆 +22%",
      avgOrder: "重庆 +13%",
    },
    {
      order: "苏州 +6%",
      shop: "苏州 +6%",
      rider: "苏州 +15%",
      newShop: "苏州 -17%",
      avgOrder: "苏州 -23%",
    },
    {
      order: "无锡 +8%",
      shop: "无锡 -11%",
      rider: "无锡 +23%",
      newShop: "无锡 -22%",
      avgOrder: "无锡 +6%",
    },
    {
      order: "常州 -11%",
      shop: "常州 +11%",
      rider: "常州 -13%",
      newShop: "常州 +5%",
      avgOrder: "常州 -17%",
    },
    {
      order: "温州 -9%",
      shop: "温州 +16%",
      rider: "温州 -8%",
      newShop: "温州 +16%",
      avgOrder: "温州 +18%",
    },
    {
      order: "哈尔滨 +11%",
      shop: "哈尔滨 +15%",
      rider: "哈尔滨 +13%",
      newShop: "哈尔滨 -11%",
      avgOrder: "哈尔滨 +3%",
    },
    {
      order: "长春 +15%",
      shop: "长春 +23%",
      rider: "长春 -11%",
      newShop: "长春 +5%",
      avgOrder: "长春 -12%",
    },
    {
      order: "大连 -9%",
      shop: "大连 +7%",
      rider: "大连 -3%",
      newShop: "大连 +11%",
      avgOrder: "大连 -16%",
    },
    {
      order: "沈阳 -12%",
      shop: "沈阳 +11%",
      rider: "沈阳 +13%",
      newShop: "沈阳 -25%",
      avgOrder: "沈阳 -15%",
    },
    {
      order: "拉萨 +25%",
      shop: "拉萨 +15%",
      rider: "拉萨 +16%",
      newShop: "拉萨 -14%",
      avgOrder: "拉萨 +8%",
    },
    {
      order: "呼和浩特 +13%",
      shop: "呼和浩特 +24%",
      rider: "呼和浩特 -21%",
      newShop: "呼和浩特 +14%",
      avgOrder: "呼和浩特 +13%",
    },
    {
      order: "武汉 -7%",
      shop: "武汉 -21%",
      rider: "武汉 -16%",
      newShop: "武汉 +21%",
      avgOrder: "武汉 -12%",
    },
    {
      order: "南宁 +16%",
      shop: "南宁 +6%",
      rider: "南宁 +16%",
      newShop: "南宁 +14%",
      avgOrder: "南宁 -20%",
    },
  ],
  areaTop: [
    {
      city: "北京",
      rate: "+11%",
      shop: [
        {
          shop: "必胜客",
          order: 127,
          sales: 2461,
        },
        {
          shop: "肯德基",
          order: 686,
          sales: 10241,
        },
        {
          shop: "麦当劳",
          order: 373,
          sales: 6318,
        },
        {
          shop: "海底捞",
          order: 442,
          sales: 4702,
        },
      ],
    },
    {
      city: "上海",
      rate: "-15%",
      shop: [
        {
          shop: "必胜客",
          order: 470,
          sales: 7139,
        },
        {
          shop: "肯德基",
          order: 329,
          sales: 3451,
        },
        {
          shop: "麦当劳",
          order: 161,
          sales: 2706,
        },
        {
          shop: "海底捞",
          order: 342,
          sales: 6296,
        },
      ],
    },
    {
      city: "广州",
      rate: "+10%",
      shop: [
        {
          shop: "必胜客",
          order: 571,
          sales: 6572,
        },
        {
          shop: "肯德基",
          order: 386,
          sales: 4554,
        },
        {
          shop: "麦当劳",
          order: 20,
          sales: 203,
        },
        {
          shop: "海底捞",
          order: 791,
          sales: 15353,
        },
      ],
    },
    {
      city: "深圳",
      rate: "+5%",
      shop: [
        {
          shop: "必胜客",
          order: 328,
          sales: 6481,
        },
        {
          shop: "肯德基",
          order: 744,
          sales: 9798,
        },
        {
          shop: "麦当劳",
          order: 482,
          sales: 4882,
        },
        {
          shop: "海底捞",
          order: 526,
          sales: 8153,
        },
      ],
    },
    {
      city: "南京",
      rate: "-18%",
      shop: [
        {
          shop: "必胜客",
          order: 889,
          sales: 15095,
        },
        {
          shop: "肯德基",
          order: 986,
          sales: 11191,
        },
        {
          shop: "麦当劳",
          order: 13,
          sales: 192,
        },
        {
          shop: "海底捞",
          order: 210,
          sales: 4023,
        },
      ],
    },
    {
      city: "杭州",
      rate: "-16%",
      shop: [
        {
          shop: "必胜客",
          order: 911,
          sales: 14667,
        },
        {
          shop: "肯德基",
          order: 890,
          sales: 16927,
        },
        {
          shop: "麦当劳",
          order: 772,
          sales: 15023,
        },
        {
          shop: "海底捞",
          order: 194,
          sales: 2102,
        },
      ],
    },
    {
      city: "合肥",
      rate: "+21%",
      shop: [
        {
          shop: "必胜客",
          order: 386,
          sales: 6538,
        },
        {
          shop: "肯德基",
          order: 638,
          sales: 7151,
        },
        {
          shop: "麦当劳",
          order: 685,
          sales: 10747,
        },
        {
          shop: "海底捞",
          order: 1,
          sales: 11,
        },
      ],
    },
    {
      city: "济南",
      rate: "+1%",
      shop: [
        {
          shop: "必胜客",
          order: 550,
          sales: 6143,
        },
        {
          shop: "肯德基",
          order: 63,
          sales: 1224,
        },
        {
          shop: "麦当劳",
          order: 737,
          sales: 11032,
        },
        {
          shop: "海底捞",
          order: 884,
          sales: 9962,
        },
      ],
    },
    {
      city: "太原",
      rate: "+2%",
      shop: [
        {
          shop: "必胜客",
          order: 715,
          sales: 12412,
        },
        {
          shop: "肯德基",
          order: 857,
          sales: 16317,
        },
        {
          shop: "麦当劳",
          order: 573,
          sales: 11001,
        },
        {
          shop: "海底捞",
          order: 716,
          sales: 7174,
        },
      ],
    },
    {
      city: "成都",
      rate: "+14%",
      shop: [
        {
          shop: "必胜客",
          order: 639,
          sales: 7060,
        },
        {
          shop: "肯德基",
          order: 797,
          sales: 15509,
        },
        {
          shop: "麦当劳",
          order: 223,
          sales: 3659,
        },
        {
          shop: "海底捞",
          order: 859,
          sales: 14267,
        },
      ],
    },
    {
      city: "重庆",
      rate: "+18%",
      shop: [
        {
          shop: "必胜客",
          order: 811,
          sales: 11426,
        },
        {
          shop: "肯德基",
          order: 872,
          sales: 15896,
        },
        {
          shop: "麦当劳",
          order: 905,
          sales: 13249,
        },
        {
          shop: "海底捞",
          order: 764,
          sales: 12124,
        },
      ],
    },
    {
      city: "苏州",
      rate: "+3%",
      shop: [
        {
          shop: "必胜客",
          order: 616,
          sales: 11069,
        },
        {
          shop: "肯德基",
          order: 235,
          sales: 2432,
        },
        {
          shop: "麦当劳",
          order: 719,
          sales: 12072,
        },
        {
          shop: "海底捞",
          order: 495,
          sales: 9162,
        },
      ],
    },
    {
      city: "无锡",
      rate: "+14%",
      shop: [
        {
          shop: "必胜客",
          order: 126,
          sales: 1988,
        },
        {
          shop: "肯德基",
          order: 156,
          sales: 2304,
        },
        {
          shop: "麦当劳",
          order: 29,
          sales: 504,
        },
        {
          shop: "海底捞",
          order: 166,
          sales: 1995,
        },
      ],
    },
    {
      city: "常州",
      rate: "-3%",
      shop: [
        {
          shop: "必胜客",
          order: 382,
          sales: 7330,
        },
        {
          shop: "肯德基",
          order: 779,
          sales: 13889,
        },
        {
          shop: "麦当劳",
          order: 170,
          sales: 2929,
        },
        {
          shop: "海底捞",
          order: 384,
          sales: 4389,
        },
      ],
    },
    {
      city: "温州",
      rate: "+23%",
      shop: [
        {
          shop: "必胜客",
          order: 988,
          sales: 10541,
        },
        {
          shop: "肯德基",
          order: 151,
          sales: 2837,
        },
        {
          shop: "麦当劳",
          order: 549,
          sales: 10161,
        },
        {
          shop: "海底捞",
          order: 875,
          sales: 14043,
        },
      ],
    },
    {
      city: "哈尔滨",
      rate: "-18%",
      shop: [
        {
          shop: "必胜客",
          order: 159,
          sales: 2672,
        },
        {
          shop: "肯德基",
          order: 172,
          sales: 2905,
        },
        {
          shop: "麦当劳",
          order: 293,
          sales: 3967,
        },
        {
          shop: "海底捞",
          order: 10,
          sales: 134,
        },
      ],
    },
    {
      city: "长春",
      rate: "-12%",
      shop: [
        {
          shop: "必胜客",
          order: 521,
          sales: 7236,
        },
        {
          shop: "肯德基",
          order: 690,
          sales: 12247,
        },
        {
          shop: "麦当劳",
          order: 258,
          sales: 2997,
        },
        {
          shop: "海底捞",
          order: 318,
          sales: 4502,
        },
      ],
    },
    {
      city: "大连",
      rate: "-7%",
      shop: [
        {
          shop: "必胜客",
          order: 986,
          sales: 15066,
        },
        {
          shop: "肯德基",
          order: 137,
          sales: 1872,
        },
        {
          shop: "麦当劳",
          order: 808,
          sales: 14010,
        },
        {
          shop: "海底捞",
          order: 300,
          sales: 3597,
        },
      ],
    },
    {
      city: "沈阳",
      rate: "+22%",
      shop: [
        {
          shop: "必胜客",
          order: 757,
          sales: 13194,
        },
        {
          shop: "肯德基",
          order: 779,
          sales: 9254,
        },
        {
          shop: "麦当劳",
          order: 185,
          sales: 3230,
        },
        {
          shop: "海底捞",
          order: 931,
          sales: 15836,
        },
      ],
    },
    {
      city: "拉萨",
      rate: "-14%",
      shop: [
        {
          shop: "必胜客",
          order: 445,
          sales: 6029,
        },
        {
          shop: "肯德基",
          order: 846,
          sales: 14085,
        },
        {
          shop: "麦当劳",
          order: 139,
          sales: 2760,
        },
        {
          shop: "海底捞",
          order: 360,
          sales: 5871,
        },
      ],
    },
    {
      city: "呼和浩特",
      rate: "-7%",
      shop: [
        {
          shop: "必胜客",
          order: 685,
          sales: 13583,
        },
        {
          shop: "肯德基",
          order: 530,
          sales: 7965,
        },
        {
          shop: "麦当劳",
          order: 509,
          sales: 6561,
        },
        {
          shop: "海底捞",
          order: 689,
          sales: 7951,
        },
      ],
    },
    {
      city: "武汉",
      rate: "+13%",
      shop: [
        {
          shop: "必胜客",
          order: 112,
          sales: 2075,
        },
        {
          shop: "肯德基",
          order: 431,
          sales: 7555,
        },
        {
          shop: "麦当劳",
          order: 997,
          sales: 15563,
        },
        {
          shop: "海底捞",
          order: 829,
          sales: 12260,
        },
      ],
    },
    {
      city: "南宁",
      rate: "-7%",
      shop: [
        {
          shop: "必胜客",
          order: 435,
          sales: 7273,
        },
        {
          shop: "肯德基",
          order: 486,
          sales: 4937,
        },
        {
          shop: "麦当劳",
          order: 917,
          sales: 15524,
        },
        {
          shop: "海底捞",
          order: 337,
          sales: 5900,
        },
      ],
    },
  ],
  realTimeOrder: 1027,
  growthLastDay: "123.61",
  growthLastMonth: "179.52",
  wordCloud: [
    {
      word: "北京",
      count: 2209,
      user: 966,
    },
    {
      word: "上海",
      count: 616,
      user: 505,
    },
    {
      word: "广州",
      count: 9158,
      user: 6341,
    },
    {
      word: "深圳",
      count: 8058,
      user: 6681,
    },
    {
      word: "南京",
      count: 4849,
      user: 2033,
    },
    {
      word: "杭州",
      count: 5215,
      user: 575,
    },
    {
      word: "合肥",
      count: 1887,
      user: 1045,
    },
    {
      word: "济南",
      count: 9823,
      user: 9448,
    },
    {
      word: "太原",
      count: 8981,
      user: 4561,
    },
    {
      word: "成都",
      count: 8291,
      user: 8007,
    },
    {
      word: "重庆",
      count: 5724,
      user: 2795,
    },
    {
      word: "苏州",
      count: 9984,
      user: 5122,
    },
    {
      word: "无锡",
      count: 1171,
      user: 1155,
    },
    {
      word: "常州",
      count: 2217,
      user: 496,
    },
    {
      word: "温州",
      count: 1471,
      user: 861,
    },
    {
      word: "哈尔滨",
      count: 7641,
      user: 1920,
    },
    {
      word: "长春",
      count: 1645,
      user: 1261,
    },
    {
      word: "大连",
      count: 6087,
      user: 2603,
    },
    {
      word: "沈阳",
      count: 530,
      user: 475,
    },
    {
      word: "拉萨",
      count: 2940,
      user: 1692,
    },
    {
      word: "呼和浩特",
      count: 9473,
      user: 8640,
    },
    {
      word: "武汉",
      count: 8184,
      user: 3158,
    },
    {
      word: "南宁",
      count: 4156,
      user: 4145,
    },
    {
      word: "必胜客",
      count: 2051,
      user: 1332,
    },
    {
      word: "肯德基",
      count: 6231,
      user: 3123,
    },
    {
      word: "麦当劳",
      count: 7807,
      user: 7593,
    },
    {
      word: "海底捞",
      count: 1952,
      user: 1788,
    },
    {
      word: "美食",
      count: 9922,
      user: 2849,
    },
    {
      word: "商超",
      count: 726,
      user: 61,
    },
    {
      word: "水果",
      count: 2258,
      user: 1097,
    },
    {
      word: "跑腿",
      count: 7557,
      user: 4684,
    },
    {
      word: "送药",
      count: 7521,
      user: 921,
    },
    {
      word: "烩饭",
      count: 5980,
      user: 4585,
    },
    {
      word: "面条",
      count: 9913,
      user: 4971,
    },
    {
      word: "小龙虾",
      count: 7969,
      user: 7413,
    },
    {
      word: "牛肉",
      count: 373,
      user: 58,
    },
    {
      word: "鸡腿",
      count: 494,
      user: 208,
    },
    {
      word: "全家桶",
      count: 5178,
      user: 2708,
    },
    {
      word: "麦乐鸡",
      count: 4895,
      user: 996,
    },
    {
      word: "炭烤",
      count: 3456,
      user: 1377,
    },
    {
      word: "麻辣",
      count: 7280,
      user: 1279,
    },
    {
      word: "冒菜",
      count: 6173,
      user: 3599,
    },
  ],
  salesRadar: {
    indicator: [
      { name: "订单量", max: 6500 },
      { name: "骑手量", max: 16000 },
      { name: "访问量", max: 30000 },
      { name: "客服", max: 38000 },
      { name: "配送", max: 52000 },
      { name: "热度", max: 25000 },
    ],
    data: [
      { value: [4300, 10000, 28000, 35000, 50000, 19000], name: "预期" },
      { value: [5000, 14000, 28000, 31000, 42000, 21000], name: "实际" },
    ],
  },
  salesSun: [
    {
      name: "小食",
      children: [
        {
          name: "奶茶",
          children: [
            {
              name: "5☆",
              children: [{ name: "港式奶茶" }, { name: "珍珠奶茶" }],
            },
          ],
        },
        {
          name: "串串",
          children: [
            { name: "5☆", children: [{ name: "羊肉串" }] },
            { name: "4☆", children: [{ name: "牛肉串" }, { name: "猪肉串" }] },
            { name: "3☆", children: [{ name: "其他" }] },
          ],
        },
      ],
    },
    {
      name: "主食",
      children: [
        {
          name: "米饭",
          children: [
            { name: "5☆", children: [{ name: "王者炒饭" }] },
            { name: "4☆", children: [{ name: "套餐" }, { name: "轻食" }] },
          ],
        },
        {
          name: "粥",
          children: [
            { name: "5☆", children: [{ name: "牛肉粥" }] },
            { name: "4☆", children: [{ name: "白米粥" }, { name: "红豆粥" }] },
          ],
        },
        {
          name: "西餐",
          children: [
            { name: "5☆", children: [{ name: "汉堡" }] },
            { name: "4☆", children: [{ name: "薯条" }, { name: "薯饼" }] },
          ],
        },
      ],
    },
  ],
  businessMapData: [
    {
      name: "黄金用户",
      children: [
        {
          name: "武汉",
          value: [114.31, 30.52],
        },
        {
          name: "丹东",
          value: [124.37, 40.13],
        },
        {
          name: "张家口",
          value: [114.87, 40.82],
        },
        {
          name: "深圳",
          value: [114.07, 22.62],
        },
      ],
    },
    {
      name: "白金用户",
      children: [
        {
          name: "金华",
          value: [119.64, 29.12],
        },
        {
          name: "西安",
          value: [108.95, 34.27],
        },
      ],
    },
    {
      name: "砖石用户",
      children: [
        {
          name: "成都",
          value: [104.06, 30.67],
        },
      ],
    },
  ],
} as any;

export default chartData;
